...
actual fun rememberZhihuAnswerPublisher(): ZhihuAnswerPublisher = remember {
    object : ZhihuAnswerPublisher {
        ...
        override suspend fun uploadImage(bytes: ByteArray, mimeType: String?, fileName: String?): UploadedZhihuImage = throw UnsupportedOperationException("桌面端暂不支持上传图片")

        override suspend fun uploadImageFromUrl(url: String): UploadedZhihuImage = throw UnsupportedOperationException("桌面端暂不支持上传图片")
        ...
        override suspend fun patchDraft(
            ...
            html: String,
            tocEnabled: Boolean,
        ): Unit = throw UnsupportedOperationException("桌面端暂不支持发布/编辑知乎回答")
